...
int main( int argc, char** argv )
{
  ...
  //ros::Publisher spheres_pub = n.advertise<visualization_msgs::Marker>("visualization_marker", 1);
  ...
  while (ros::ok())
  {
	...
	while (!success) {
		try {
			...
			listener.waitForTransform("/base_footprint",std::string("/head_") + char('0' + person),now,ros::Duration(1.0));
			listener.lookupTransform("/base_footprint",std::string("/head_") + char('0' + person),now,transform);
    ...
    //display_sphere.type = shape_sphere;

    // Set the marker action.  Options are ADD and DELETE
    marker.action = visualization_msgs::Marker::ADD;
    connecting_givenframes.action = visualization_msgs::Marker::ADD;
    //display_sphere.action = visualization_msgs::Marker::ADD;
	...
	//display_sphere.pose.position.x = transform.getOrigin().getX();
	//display_sphere.pose.position.y = transform.getOrigin().getY();
	//display_sphere.pose.position.z = transform.getOrigin().getZ();
	//display_sphere.points[1].x = transform_.getOrigin().getX();
	//connecting_givenframes.points[1].y = transform_.getOrigin().getY();
	//connecting_givenframes.points[1].z = transform_.getOrigin().getZ();
