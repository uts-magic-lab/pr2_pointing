...
int main( int argc, char** argv )
{
  ...
  while (ros::ok())
  {
	...
	try
	{
		...
		listener.waitForTransform("/base_footprint","/origin_1",now,ros::Duration(1.0));
		listener.lookupTransform("/base_footprint","/origin_1",now,transform);
			
		ros::Time now_ = ros::Time::now();
		listener_.waitForTransform("/base_footprint","/origin_2",now_,ros::Duration(1.0));
		listener_.lookupTransform("/base_footprint","/origin_2",now_,transform_);
			
			
			std::cout
					<< now << "base_footprint->origin_1"
			  	    << transform.stamp_ << "  "
